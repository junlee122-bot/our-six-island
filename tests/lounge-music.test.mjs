...
test('the music box plays outside; a room plays its file, or its generative piece', () => {
  assert.deepEqual(musicMix(scene({}), true, ready([])), { channel: 1, box: 0.2, track: null, synth: null });
  assert.equal(musicMix(scene({ night: true }), true, ready([])).box, 0.16);
  // In the casino without a file (or while it loads) the casino piece plays.
  ...
  // Once loaded, the file takes over.
