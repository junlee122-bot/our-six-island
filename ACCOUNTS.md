...
현재 원장은 최대 10,000개 게임 기록을 보관합니다. 10,000건에 도달하면 새 게임과 재대결을 거부합니다(잔액은 초기화하지 않음). world의 6MB 제한(`hh_world_commit`)은 성격이 다릅니다. 도달하면 새 게임뿐 아니라 나가기, 진행 중인 판의 행동, 접속 갱신까지 **모든 world 변경이 실패**하고 예약금도 묶입니다. 7인 게임 위주라면 10,000건보다 6MB가 먼저 올 수 있습니다. 또 기록이 1만 건 근처면 요청당 CPU가 약 0.45초로 늘어납니다. 장기 운영 시 종료 기록 보관 방식을 확장해야 합니다. Supabase의 실제 사용량과 요금은 해당 프로젝트 대시보드에서 확인합니다.
