...
## 장소 배경음 · 별빛 카지노·범마을 회관 (2026-09-26)

카지노와 회관은 들어가면(그 방의 테이블에 앉아 있을 때 포함) 자기 음악을 틉니다. 우선순위는 **① 넣어 둔 음악 파일 → ② 코드로 합성한 방 음악**이고, 마을·개인 방은 기존 오르골 그대로입니다. 장소가 바뀌면(아이리스 전환) 약 1초에 걸쳐 교차 페이드되고, 음악 켜기/끄기·음악 볼륨·Esc 메뉴 감쇠(0.35배)를 모두 따릅니다. 설정 → 소리의 **게임 중 배경음**(기본 켜짐)이 켜져 있으면 테이블에서도 음악이 0.4배로 작게 이어지고, 끄면 예전처럼 테이블에서 음악이 빠집니다.

### Suno 등으로 만든 곡 넣기

| 장소 | 파일 (둘 다 있으면 브라우저가 재생할 수 있는 첫 파일) |
| --- | --- |
| 별빛 카지노 | `public/assets/lounge/music/casino.ogg`, `casino.mp3` |
| 범마을 회관 | `public/assets/lounge/music/hall.ogg`, `hall.mp3` |

- 파일만 넣으면 됩니다. 매니페스트는 `app/lounge-music-tracks.ts`이고, `npm run build:pages`(데스크톱 빌드 포함)가 있는 파일만 내용 해시 이름으로 복사하며 없는 파일은 빈 경로로 바꿉니다. 게임은 그 장소에 처음 들어갈 때 파일을 받아(지연 로딩) 디코딩하고, 받는 동안과 파일이 없을 때는 합성 음악이 대신 나옵니다. 90초 넘게 안 들은 곡은 메모리에서 내립니다.
- 형식: **OGG Vorbis 권장**(끝과 처음이 샘플 단위로 붙어 루프 이음매가 없음) + Safari용 **MP3** 한 벌. 48kHz(또는 44.1kHz) 스테레오, OGG q5·MP3 192kbps 정도. 파일당 12MB를 넘으면 빌드가 멈춥니다.
- 길이: 1분 30초~3분. 재생은 디코딩한 버퍼를 `AudioBufferSourceNode`로 반복하므로 1분에 약 23MB(48kHz 스테레오)의 메모리를 씁니다.
- 음량: **통합 -16 LUFS, 트루 피크 -1.5 dBTP 이하**로 맞춰 주세요. 게임이 `TRACK_LEVEL`(0.08, 약 -22dB)을 곱해 오르골(약 -39 LUFS)과 비슷한 크기로 들리게 합니다.
  `ffmpeg -i in.wav -af loudnorm=I=-16:TP=-1.5:LRA=11 -ar 48000 -c:a libvorbis -q:a 5 casino.ogg` (MP3는 `-c:a libmp3lame -b:a 192k`)
- 루프 지점: 기본은 파일 전체를 반복합니다. 곡이 끝에서 처음으로 자연스럽게 이어지도록 잘라 두는 것이 가장 좋고, 인트로·아웃트로를 빼고 싶으면 `app/lounge-music-tracks.ts`의 그 장소에 `loopStart`/`loopEnd`(초)를 적습니다(처음 한 번은 0초부터 재생, 이후 그 구간 반복). MP3는 인코더가 앞뒤에 짧은 무음을 붙이므로 이음매가 들리면 OGG를 쓰거나 `loopEnd`를 조금 앞당기세요.
- 곡을 넣으면 생성 기록(서비스, 날짜, 프롬프트, 이용 약관상 권리)을 같은 폴더의 `casino.prompt.txt`처럼 남기고, 기존 영화 음악의 멜로디를 따라 하거나 제목·아티스트 이름을 프롬프트에 쓰지 않습니다(분위기만: 느와르 탱고, 반도네온, 하바네라 리듬 등).

### 합성 방 음악 (파일이 없을 때)

오디오 파일 없이 WebAudio로 만드는 원곡입니다. 영화 '타짜'풍의 긴장감 있는 한국 느와르 도박 분위기만 참고했고, 실제 영화 음악의 선율·주제는 쓰지 않았습니다(선율은 코드 톤 위의 시드 기반 무작위 진행). 악보는 `app/lounge-music-score.ts`(순수 데이터, `tests/lounge-music-score.test.mjs`), 악기와 스케줄러는 `app/lounge-music-synth.ts`입니다.

- **별빛 카지노** — D 단조 느와르 탱고, 104 bpm 4/4. A(8마디, D–C♯–C–B로 내려가는 베이스 라인 클리셰) · B(8, B♭maj7–Gm7–Em7♭5–A7♭9…E♭maj7 나폴리 색) · A′(8, A 선율 6마디를 되받고 새 종지) · C(8, A 페달 위 B♭/A 반음 긴장) · 턴어라운드(4) = 36마디, 두 번째 바퀴부터 A-B-C-A′-T / A-A′-B-C-T 중 무작위. 피치카토 업라이트 베이스가 A에서 하바네라(점8분·16분·8분), B에서 워킹, C·T에서 3-3-2 마르카토. 반도네온(디튠 톱니파 2개 → 밴드패스 → 공유 벨로즈 트레몰로)이 코드를, 같은 음색의 리드가 선율을 맡고, 딸림화음 마디에서는 화성단음계 증2도(B♭–C♯) 음형이 나옵니다. B에는 바이올린 긴 음·16분 현 오스티나토·약음 금관 스탭, C에는 A–B♭ 스피카토 오스티나토와 낮은 피아노 클러스터, 턴어라운드에는 반음씩 내려오는 피아노 16분 음형. 브러시 스윕·브러시 스네어(2·4박)·림샷·부드러운 킥, 악절 끝 무작위 필인과 폼 머리 앞 스웰.
- **범마을 회관** — 같은 영화의 국악 변주, 12/8 굿거리(점4분 66). 장구 덩·기덕·쿵·더러러(궁편 = 사인 피치 하강 + 타격, 채편 = 대나무 같은 밴드패스 잡음), 가야금풍 뜯는 소리(밝은 톱니파 어택이 빨리 사라지는 사인)로 D 계면조(D F G A C) 선율과 농현(떠는 소리 = 비브라토 곡선, 추성 = 아래서 밀어 올림, 퇴성 = 끝에서 온음 꺾기), B 절의 대금풍 긴 음(숨소리 잡음 + 느린 비브라토), C 절의 산조풍 여백, 턴어라운드의 5음 하행 런과 휘모리 필. 밑에는 카지노와 같은 피치카토 베이스·반도네온·브러시가 깔려 두 방이 한 영화처럼 들립니다.
- **테이블 연출**(홀덤·블랙잭, `useTableSounds`): 판이 진행되는 동안 긴장 레이어(심장 박동 lub-dub + 딸림음 A 펄스, 마디마다 필터가 열리며 커지고 높아지면 박동이 두 배)가 합성 음악에 더해지고, 올인·블랙잭·큰 판(빅 블라인드 20배 이상)에서는 1~2초짜리 D조 스팅이 효과음 채널로 한 번 울립니다(카드·칩 소리 크기 수준).
- 성능: 악기 버스·필터·잔향(1.6초 합성 임펄스)·LFO는 한 번 만들어 재사용하고, 음표마다 오실레이터와 엔벨로프 게인만 만듭니다. 기존 오르골과 같은 150ms 타이머·0.6초 선행 스케줄러를 쓰고, 탭이 숨겨지면 컨텍스트를 멈추며 돌아오면 밀린 음을 몰아 치지 않고 지금부터 이어갑니다. 모든 음은 어택·릴리스 엔벨로프로 시작·끝나고(클릭 없음), 출력 앞에 리미터(-8dB, 12:1)가 있습니다. 합성 음악의 레벨 1 출력이 약 -23 LUFS라 `PIECE_LEVEL` 0.2로 약 -37 LUFS(오르골보다 살짝 큼)에 둡니다.

## kArchive 마을 확장(밸리) 세트 · 2026-09-26
