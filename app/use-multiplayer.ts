...
export function useMultiplayer(callbacks:Callbacks){
 ...
 const [session]=useState(()=>new IslandSession({local:()=>latest.current.local(),candidate:character=>latest.current.candidate?.(character)??{...latest.current.local(),character},island:()=>latest.current.island(),arrive:p=>latest.current.arrive(p),exit:()=>latest.current.exit()}));
