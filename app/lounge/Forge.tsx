...
import { FIRST_TOOL_ROCKS, TIER_NAME, TOOL_INFO, type ToolId, type ToolTier } from '../lounge-growth-data';
...
const toolWord = (id: ToolId, tier: number) => `${TIER_NAME[tier as ToolTier]} ${TOOL_INFO[id].name}`;
...
export function ForgePanel({
  ...
  const [at, setAt] = useState(() => Math.max(0, g?.tools.findIndex((t) => t.next && !t.next.why) ?? 0));
  const [confirm, setConfirm] = useState<ToolView | null>(null);
  const [lifted, setLifted] = useState<{ tool: ToolId; tier: number } | null>(null);
  const [run, busy] = useLifeAction(room, notify);
  const rackRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const id = requestAnimationFrame(() => rackRef.current?.focus({ preventScroll: true }));
    return () => cancelAnimationFrame(id);
  }, []);
  if (!life || !g)
    return (
      <Modal title="대장간" onClose={onClose} className="l-forge">
        <p className="l-ledger-empty">마을에 연결되면 대장간에 들어갈 수 있어요.</p>
      </Modal>
    );
  if (!g.forgeOpen)
    return (
      <Modal title="무너진 공방" onClose={onClose} className="l-forge l-growth" wide>
        <div className="l-forge-ruin">
          <p className="l-forge-say">
            <Glyph name="anvil" size={22} />
            <span>
              지붕이 내려앉은 옛 공방이에요. 모루는 아직 멀쩡해 보여요. 마을 친구들이 범과 나무·돌을 모으면 대장장이 <b>무쇠 아저씨</b>가 다시 불을
              지펴 준대요.
            </span>
          </p>
  ...
  const pickup = async () => {
    if (!job) return;
    const ok = await run({ kind: 'forgePickup' }, `${toolWord(job.tool, job.to)}를 받았어요!`);
    if (ok) {
      lifeSfx('fanfare');
  ...
  const onKey = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') setAt((i) => (i + (e.key === 'ArrowDown' ? 1 : -1) + tools.length) % tools.length);
  ...
  return (
    <Modal title="대장간 · 무쇠 아저씨" onClose={onClose} className="l-forge l-growth" wide>
      {/* oxlint-disable-next-line jsx-a11y/no-noninteractive-element-interactions -- ↑↓ pick a tool, Enter drops it off or picks it up. */}
      <div ref={rackRef} className="l-forge-body" tabIndex={-1} role="application" aria-roledescription="대장간" onKeyDown={onKey} data-testid="forge-panel">
        <div className="l-forge-counter">
          ...
          {lifted ? (
            <div className="l-forge-lift" data-testid="forge-lifted">
              ...
              <small>{TOOL_INFO[lifted.tool].tiers[lifted.tier as ToolTier]}</small>
              <button type="button" className="l-ink l-small" onClick={() => setLifted(null)}>
                고마워요
              </button>
            </div>
          ) : job ? (
            <div className="l-forge-job" data-ready={job.ready || undefined} data-testid="forge-job">
              ...
              <span>
                ...
                <small>{job.ready ? '다 됐어요 · 지금 찾을 수 있어요' : `${untilText(job.readyAt - now)} 뒤 (아침 6시)`}</small>
              </span>
              {job.ready && (
                <button type="button" className="l-leaf" disabled={busy} onClick={() => void pickup()} data-testid="forge-pickup">
          ...
          {g.gift.available ? (
            <div className="l-forge-letter" data-testid="forge-gift">
              ...
              <span>
                <b>촌장님 편지</b> 마을 바위를 {FIRST_TOOL_ROCKS}개 넘게 깼다지? 첫 도구는 마을이 선물할게요. 구리 곡괭이를 받아 가요.
              </span>
              <button
                type="button"
                ...
                disabled={busy}
                onClick={() =>
                  void run({ kind: 'forgeGift' }, '구리 곡괭이를 선물받았어요!').then((ok) => {
                    if (ok) {
                      lifeSfx('fanfare');
          ...
          ) : !g.gift.claimed && tools.find((t) => t.id === 'pickaxe')?.tier === 1 ? (
            <p className="l-ledger-aside">
              <Glyph name="book" size={16} /> 촌장님 편지: 마을 바위 {FIRST_TOOL_ROCKS}개를 깨면 구리 곡괭이를 선물해요 ({Math.min(g.gift.rocks, FIRST_TOOL_ROCKS)}/{FIRST_TOOL_ROCKS})
            </p>
          ) : null}
        </div>
            ...
            return (
              <li key={t.id}>
                <button
                  ...
                  onClick={() => setAt(i)}
                  onDoubleClick={() => !job && next && !next.why && setConfirm(t)}
        ...
        <section className="l-forge-detail" aria-live="polite">
          ...
          {tool.next?.mats && (
            <ul className="l-tool-cost">
              <li data-short={view.wallet.balance < (tool.next.beom ?? 0) || undefined}>
                <Glyph name="sack" size={20} /> {formatBeom(tool.next.beom ?? 0)}
              </li>
              {Object.entries(tool.next.mats).map(([id, n]) => (
                <li key={id} data-short={(life.me.inv?.[id] ?? 0) < n || undefined}>
                  <ItemIcon id={id} size={22} /> {ITEM_BY_ID[id]?.name} {Math.min(life.me.inv?.[id] ?? 0, n)}/{n}
                </li>
              ))}
            </ul>
          )}
          {tool.next?.why && !tool.next.shop ? <p className="l-why">{tool.next.why}</p> : null}
          <div className="l-ledger-row-actions">
            <button
              type="button"
              className="l-leaf"
              disabled={busy || !!job || !tool.next || !!tool.next.why || view.wallet.balance < (tool.next.beom ?? 0)}
            ...
            {onGrowth && (
              <button type="button" className="l-ink l-small" onClick={onGrowth}>
                성장 수첩
              </button>
            )}
          </div>
          <p className="l-ledger-keys" aria-hidden="true">
            <kbd>↑↓</kbd> 도구 고르기 <kbd>Enter</kbd> 맡기기·찾기 <kbd>Esc</kbd> 나가기
          </p>
        </section>
      </div>
      {confirm && confirm.next && (
        <ConfirmModal
          ...
          body={
            <>
              <b>{toolWord(confirm.id, confirm.next.to)}</b>로 두드려 줄게. <b>{formatBeom(confirm.next.beom ?? 0)}</b>
              {Object.entries(confirm.next.mats ?? {}).map(([id, n]) => ` · ${ITEM_BY_ID[id]?.name} ${n}개`)}가 들어. 내일 아침 6시에 찾아가고, 그동안은
              지금 도구로 일하면 돼.
            </>
          }
          confirmLabel="맡기기"
          ...
          onConfirm={async () => {
            const ok = await run({ kind: 'forge', tool: confirm.id }, `${josa(TOOL_INFO[confirm.id].name, '을/를')} 맡겼어요. 내일 아침 6시에 찾아가요.`);
            if (ok) lifeSfx('anvil');
            return ok;
          }}
        />
