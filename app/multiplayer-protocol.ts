...
export type SharedIsland={placed:Save['placed'];night:boolean};
...
export function readPlayer(value:unknown,id?:string):OnlinePlayer|null{
 ...
 return {id:key,name,character:p.character,x:p.x,y:p.y,facing:p.facing===-1?-1:1,
...
export function visitingSpawn(host:Point){for(const offset of [{x:75,y:45},{x:-75,y:45},{x:0,y:80},{x:0,y:0}]){const p={x:host.x+offset.x,y:host.y+offset.y};if(walkable(p))return p;}return {...SPAWN};}
