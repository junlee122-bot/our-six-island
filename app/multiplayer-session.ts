...
export type SessionView={status:'offline'|'connecting'|'selecting'|'connected'|'error';role:'host'|'guest'|null;room:string;selfId:string;players:OnlinePlayer[];messages:ChatLine[];error:string;claiming:number|null};
export type OnlineScene={players:OnlinePlayer[];island:SharedIsland|null;selfId:string;visiting:boolean};
type Callbacks={local:()=>LocalPlayer;candidate?:(character:number)=>LocalPlayer;island:()=>SharedIsland;arrive:(host:OnlinePlayer)=>void;exit:()=>void;diagnostic?:(message:string)=>void};
const emptyView=():SessionView=>({status:'offline',role:null,room:'',selfId:'',players:[],messages:[],error:'',claiming:null});
...
export class IslandSession {
 ...
 private character:number|null=null;
 ...
 private localPlayer(character=this.character??this.requested??this.callbacks.local().character){return readPlayer({...this.callbacks.candidate?.(character)??this.callbacks.local(),character},this.view.selfId);}
 private lobby(id:string,error='',rejectedCharacter?:number){const pending=this.pending.get(id);if(pending)this.room?.send(id,{v:1,type:'lobby',selection:1,hostId:this.view.selfId,nonce:pending.nonce,players:[...this.members.values()],sentAt:Date.now(),error,rejectedCharacter});}
 private refreshLobbies(){for(const [id,pending] of this.pending)if(Date.now()-pending.time<=15000)this.lobby(id);}
 claimCharacter(character:number){
  if(this.view.status!=='selecting'||this.view.claiming!==null||!Number.isInteger(character)||character<0||character>=MAX_PLAYERS)return false;
  if(this.view.players.some(p=>p.character===character)){this.update({error:'이미 선택된 캐릭터예요. 다른 친구를 골라 주세요.'});return false;}
  this.requested=character;this.lastClaim=0;this.update({claiming:character,error:''});this.tick();return true;
 }
 ...
 leave(error=''){
  ...
  this.character=null;this.requested=null;this.nonce='';this.lastClaim=0;
 ...
 async start(role:'host'|'guest',input=''){
  ...
  try{
   ...
   if(role==='host'){
    ...
    this.character=player.character;this.members.set(room.id,player);this.stopDeadline();this.update({status:'connected'});this.roster([...this.members.values()]);
 ...
 private removeGuest(id:string){
  ...
  this.roster([...this.members.values()]);this.hostChat('여섯섬',player.name+' 님이 섬을 떠났어요.',true);this.broadcast(this.worldPacket());this.refreshLobbies();
 }
 ...
 private receiveGuest(id:string,data:unknown){
  ...
  if(packet.type==='hello'){
   ...
   if(packet.selection!==1){this.room?.send(id,{v:1,type:'reject',reason:'캐릭터 선택 기능이 업데이트됐어요. 새로고침한 뒤 다시 참가해 주세요.'});return;}
   ...
   challenge.time=Date.now();this.pending.delete(id);this.pending.set(id,challenge);
   if(this.pending.size>16)this.pending.delete(this.pending.keys().next().value!);
   this.lobby(id);return;
  }
  if(packet.type==='join'){
   if(this.members.has(id)){this.room?.send(id,this.worldPacket());return;}
   const pending=this.pending.get(id);if(!pending||pending.nonce!==packet.nonce||Date.now()-pending.time>15000){this.lobby(id);return;}
   const player=readPlayer(packet.player,id);if(!player)return;
   if([...this.members.values()].some(p=>p.character===player.character)){this.lobby(id,'다른 사람이 먼저 선택했어요. 남아 있는 캐릭터를 골라 주세요.',player.character);return;}
   if(this.members.size>=MAX_PLAYERS){this.lobby(id,'방이 가득 찼어요. 친구가 나가면 캐릭터를 선택할 수 있어요.',player.character);return;}
   // The host's serialized receive queue makes this availability check and claim atomic.
   this.members.set(id,player);this.pending.delete(id);this.lastSeen.set(id,Date.now());this.roster([...this.members.values()]);this.broadcast(this.worldPacket());this.refreshLobbies();this.hostChat('여섯섬',player.name+' 님이 놀러 왔어요.',true);return;
  }
  if(!this.members.has(id))return;this.lastSeen.set(id,Date.now());
  if(packet.type==='move'){
   const player=readPlayer(packet.player,id);if(player){const previous=this.members.get(id)!;if(player.character===previous.character)this.members.set(id,{...player,emote:previous.emote,emoteUntil:previous.emoteUntil});}
 ...
 private receiveHost(data:unknown){
  const packet=this.packet(data);if(!packet)return;
  if(packet.type==='challenge'&&!this.joined){this.leave('방장이 이전 버전으로 섬을 열었어요. 방장도 새로고침한 뒤 새 방을 열어 주세요.');return;}
  if(packet.type==='lobby'&&!this.joined){
   const players=readPlayers(packet.players,packet.sentAt);
   if(packet.selection!==1||packet.hostId!==PEER_PREFIX+this.view.room||!players?.some(p=>p.id===packet.hostId)||typeof packet.nonce!=='string'||!/^[a-f0-9-]{36}$/.test(packet.nonce))return;
   this.nonce=packet.nonce;this.lastHostSeen=Date.now();this.stopDeadline();this.roster(players);
   if(packet.rejectedCharacter===this.requested){this.requested=null;this.lastClaim=0;this.update({claiming:null,error:cleanText(packet.error,150)});}
   this.update({status:'selecting'});return;
  }
  ...
  if(packet.type==='world'){
   ...
   const host=players.find(p=>p.id===packet.hostId),self=players.find(p=>p.id===this.view.selfId);if(!host||!self)return;
   if(!this.joined&&self.character!==this.requested)return;
   if(this.joined&&self.character!==this.character){this.leave('캐릭터 정보가 일치하지 않아 내 섬으로 돌아왔어요.');return;}
   this.lastHostSeen=Date.now();this.scene.island=island;this.scene.visiting=true;this.roster(players);
   if(!this.joined){this.character=self.character;this.joined=true;this.stopDeadline();this.update({status:'connected',claiming:null,error:''});this.callbacks.arrive(host);}
  }else if(packet.type==='poses'&&this.joined){const players=readPlayers(packet.players,packet.sentAt);if(players?.some(p=>p.id===this.view.selfId&&p.character===this.character)&&players.some(p=>p.id===PEER_PREFIX+this.view.room)){this.lastHostSeen=Date.now();this.roster(players);}}
 ...
 private tick(){
  const now=Date.now();const local=this.localPlayer();if(!local)return;
  if(this.view.role==='host'){
   ...
   for(const [id,pending] of this.pending)if(now-pending.time>15000)this.pending.delete(id);
   ...
   if(this.view.status==='selecting'&&now-this.lastHostSeen>45000){this.leave('방장의 응답이 없어 선택 화면을 닫았어요. 다시 참가해 주세요.');return;}
   if(now-this.lastHello>2500){this.lastHello=now;this.sendHost({v:1,type:'hello',selection:1,player:local});}
   if(this.requested!==null&&this.nonce&&now-this.lastClaim>2500){this.lastClaim=now;this.sendHost({v:1,type:'join',nonce:this.nonce,player:local});}
  }else{
