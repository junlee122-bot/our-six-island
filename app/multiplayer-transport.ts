...
export class IslandRoom {
 ...
 private constructor(identity:Identity,host:boolean,code:string){
 ...
 static async create(host:boolean,code:string){const identity=await createIdentity(host);return new IslandRoom(identity,host,host?identity.code:code);}
