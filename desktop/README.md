...
app/desktop-bridge.ts        게임 코드에서 쓸 브리지 (아직 연결 안 함, 아래 "연결 지점")
...
|---|---|---|---|---|---|
| 1 | `app/lounge/FriendsModal.tsx:57-61` (`link()`), `app/multiplayer-ui.tsx:15`, `app/theater-game.tsx:34` | 초대 링크 = `location.href` + `#lounge=코드` | `tauri://localhost/…#lounge=…` 링크가 복사돼 친구가 열 수 없음 | `shareableInviteUrl('lounge', code)` 사용 → 공개 웹 주소(`WEB_URL`) 기준 링크 | 브리지에 구현, **연결은 안 함** |
| 2 | `app/lounge-game.tsx:472` (`location.hash`의 `lounge=` 읽기), `app/lounge-cloud-room.ts:365-373` (`clearInviteHash`), `app/theater-game.tsx:17`, `app/island-game.tsx:40` | 초대 링크를 브라우저로 열면 해시로 방 코드 전달 | 앱은 항상 `index.html`로 시작해서 해시가 없음 → 링크로 앱 안의 방에 들어갈 수 없음(코드 직접 입력은 가능) | `tauri-plugin-deep-link`로 `beomtadew://lounge/<코드>` 등록 + single-instance의 `deep-link` 기능으로 실행 중인 앱에 전달 → JS에서 `onOpenUrl`로 받아 `room.init(…, code)`/방 입장 호출. 웹 링크 페이지에 "앱으로 열기" 버튼 추가 | 권장만 |
...
| 5 | `app/lounge/ErrorBoundary.tsx:26-35` (`reloadOnce`), `app/lounge-game.tsx:275,1334`, `app/lounge/GameScreen.tsx:310` | 청크 로드 실패 = "새 배포가 올라옴" → 새로고침하면 해결 | 앱은 파일이 설치본에 있어 새로고침해도 같은 파일. 실패는 설치 손상일 가능성 | 앱에서는 새로고침 대신 "업데이트 확인"(`checkForDesktopUpdate`) 안내. 무한 새로고침은 기존 쿨다운으로 방지됨 | 권장만 |
...
| 8 | `app/lounge/feedback.ts:47-52` (`document.title`), `:66-71` (`tabHidden`), `app/lounge-cloud-room.ts:151,171` | 탭이 숨겨졌을 때만 제목 배지·알림, 숨김 탭은 느린 폴링 | 앱 창은 다른 창 뒤에 있어도 `visible` → 알림이 안 뜸. 제목 표시줄 반영은 플랫폼별로 다름 | 앱에서는 `desktopWindowFocused()`/`onDesktopFocusChange()`로 판단, 제목은 `setDesktopTitle()` | 브리지에 구현, 연결 안 함 |
| 9 | `app/lounge/feedback.ts:90-105`, `app/lounge/SettingsModal.tsx:42,97` | 웹 `Notification` API | notification 플러그인이 `window.Notification`을 대체해서 **그대로 동작**. 단 `onclick`(창 포커스)은 데스크톱에서 호출되지 않음 | 명시적으로 `desktopNotify()` + `requestUserAttention`(작업 표시줄 깜빡임/Dock 튕김) 사용 | 브리지에 구현 |
| 10 | `app/lounge/feedback.ts:82` (`navigator.vibrate`) | 모바일 진동 | 데스크톱엔 없음(옵셔널 체이닝이라 안전) | 없음 | 확인 |
...
## 연결 지점 (다른 작업이 끝난 뒤 게임 코드에 붙일 곳)

`app/desktop-bridge.ts`의 함수는 브라우저에선 전부 아무 일도 하지 않아서, 웹 동작을
바꾸지 않고 바로 붙일 수 있어요.

- `scripts/standalone-entry.tsx`: 시작 시 `installDesktopShortcuts()` (셸이 이미 F11을 넣었으면 건너뜀).
- `app/lounge/feedback.ts` `attention()`: `isDesktopApp()`이면 `tabHidden()` 대신
  `await desktopWindowFocused()`로 판단하고 `desktopNotify(title, body)` 호출,
  `renderTitle()`은 `setDesktopTitle(…)`.
- `app/lounge/SettingsModal.tsx`: 알림 토글에서 `requestDesktopNotificationPermission()`,
  "전체 화면 (F11)" 버튼 → `toggleDesktopFullscreen()`, "업데이트 확인" → `checkForDesktopUpdate()`.
- `app/lounge/FriendsModal.tsx` `link()`, `app/multiplayer-ui.tsx:15`, `app/theater-game.tsx:34`:
  `shareableInviteUrl('lounge' | 'room' | 'theater', code)`.
- 딥 링크를 추가하면 `lounge-game.tsx:472`의 해시 읽기 옆에서 딥 링크로 받은 코드도 처리.

## 권한 (capabilities/default.json)

`core:default`, 창 전체 화면/제목/포커스/주의 요청, `notification:default`,
